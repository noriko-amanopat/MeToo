...
foreach sample in allaut onlyju senior {
	...
	* Note that the following dataset is an output of regressions_main.do
	use "${data}MeToo_author_ppr_`sample'.dta", clear
	
	*** Placebo MeToo date
	replace metoo = (min_date>=td(01jul2014) & min_date<=td(31dec2016))
		...
		* local var new_prop_paper
		if `j'==1 {
			* glm with interactons with sex
			eststo reg`j'_frac:			glm `var' i.metoo##i.sex i.sex##i.covid_period $exper_time $network $prodty $fields [aw = inv_weight], family(binomial) link(logit) vce(cluster temp_name_orig)
			eststo mar_reg`j'_mef_fr:	margins, dydx(metoo) at(sex=(0 1)) post
			
			* glm without interactions with sex
			eststo reg`j'_all:          glm `var' i.metoo i.sex i.covid_period $exper_time $network $prodty $fields [aw = inv_weight], family(binomial) link(logit) vce(cluster temp_name_orig)
			eststo mar_reg`j'_mef_all:  margins, dydx(metoo) post
		}
		else if `j'!=1 {
			local relevant_share : word `j' of `list_relev_shares'
			* glm with interactons with sex
			eststo reg`j'_frac:			glm `var' i.metoo##i.sex i.sex##i.covid_period `relevant_share' $exper_time $network $prodty $fields [aw = inv_weight], family(binomial) link(logit) vce(cluster temp_name_orig)
			eststo mar_reg`j'_mef_fr:	margins, dydx(metoo) at(sex=(0 1)) post
			
			* glm without interactions with sex
			eststo reg`j'_all:          glm `var' i.metoo i.sex i.covid_period `relevant_share' $exper_time $network $prodty $fields [aw = inv_weight], family(binomial) link(logit) vce(cluster temp_name_orig)
			eststo mar_reg`j'_mef_all:  margins, dydx(metoo) post
		}
		local j = `j' + 1
	}
	...
	* Loop over each dependent variable
	foreach var in `depVars' {
		...
		*local var new_prop_paper
		local relevant_share : word `j' of `list_relev_shares'
		if `j' == 1 {
			eststo reg`j'b_frac:			glm `var' i.metoo##i.sex i.sex##i.covid_period `relevant_share' $exper_time $network $prodty $fields [aw = inv_weight], family(binomial) link(logit) vce(cluster temp_name_orig)
			eststo mar_reg`j'b_mef_fr:		margins, dydx(metoo) at(sex=(0 1)) post
			
			* glm without interactions with sex
			eststo reg`j'b_all:             glm `var' i.metoo i.sex i.covid_period `relevant_share' $exper_time $network $prodty $fields [aw = inv_weight], family(binomial) link(logit) vce(cluster temp_name_orig)
			eststo mar_reg`j'b_mef_all:     margins, dydx(metoo) post
		}
		else if `j'!=1 {
			local relevant_share_b : word `j' of `list_relev_shares_b'
			eststo reg`j'b_frac:			glm `var' i.metoo##i.sex i.sex##i.covid_period `relevant_share' `relevant_share_b' $exper_time $network $prodty $fields [aw = inv_weight], family(binomial) link(logit) vce(cluster temp_name_orig)
			eststo mar_reg`j'b_mef_fr:		margins, dydx(metoo) at(sex=(0 1)) post
			
			* glm without interactions with sex
			eststo reg`j'b_all:             glm `var' i.metoo i.sex i.covid_period `relevant_share' `relevant_share_b' $exper_time $network $prodty $fields [aw = inv_weight], family(binomial) link(logit) vce(cluster temp_name_orig)
			eststo mar_reg`j'b_mef_all:     margins, dydx(metoo) post
		}	
		local j = `j' + 1
	}
...
**# Bookmark #1
	...
	* Table with MEF of GLM of ALL COAUTHORS (2c-5c and 1d-5d)
	esttab mar_reg2c_mef_fr mar_reg3c_mef_fr mar_reg4c_mef_fr mar_reg5c_mef_fr mar_reg1d_mef_fr mar_reg2d_mef_fr mar_reg3d_mef_fr mar_reg4d_mef_fr mar_reg5d_mef_fr using "${output}tables_`sample'/`sample'_placebo_ALL_sexinteractions.csv", star(* 0.10 ** 0.05 *** 0.01) replace style(html) cells(b(fmt(4) star) se(fmt(4) par)) title("Sample `sample', marginal effects: Shares of ALL coauthors") mlabels("Senior" "Mid-career" "Junior" "Other" "Women" "Senior women" "Mid-career women" "Junior women" "Other women")

	* Table with MEF of GLM of ALL COAUTHORS, all genders (2c-5c and 1d-5d)
	esttab mar_reg2c_mef_all mar_reg3c_mef_all mar_reg4c_mef_all mar_reg5c_mef_all mar_reg1d_mef_all mar_reg2d_mef_all mar_reg3d_mef_all mar_reg4d_mef_all mar_reg5d_mef_all using "${output}tables_`sample'/`sample'_placebo_ALL.csv", star(* 0.10 ** 0.05 *** 0.01) replace style(html) cells(b(fmt(4) star) se(fmt(4) par)) title("Sample `sample', marginal effects: Shares of ALL coauthors, all genders") mlabels("Senior" "Mid-career" "Junior" "Other" "Women" "Senior women" "Mid-career women" "Junior women" "Other women")

	* Table with MEF of GLM of NEW COAUTHORS (1-5 and 1b-5b)
	esttab mar_reg1_mef_fr mar_reg2_mef_fr mar_reg3_mef_fr mar_reg4_mef_fr mar_reg5_mef_fr mar_reg1b_mef_fr mar_reg2b_mef_fr mar_reg3b_mef_fr mar_reg4b_mef_fr mar_reg5b_mef_fr using "${output}tables_`sample'/`sample'_placebo_NEW_sexinteractions.csv", star(* 0.10 ** 0.05 *** 0.01) replace style(html) cells(b(fmt(4) star) se(fmt(4) par)) title("Sample `sample', marginal effects: Shares of NEW coauthors") mlabels("New coauthors" "New senior" "New mid-career" "New junior" "New other" "New women" "New senior women" "New mid-career women" "New junior women" "New other women")

	* Table with MEF of GLM of NEW COAUTHORS, all genders (1-5 and 1b-5b)
	esttab mar_reg1_mef_all mar_reg2_mef_all mar_reg3_mef_all mar_reg4_mef_all mar_reg5_mef_all mar_reg1b_mef_all mar_reg2b_mef_all mar_reg3b_mef_all mar_reg4b_mef_all mar_reg5b_mef_all using "${output}tables_`sample'/`sample'_placebo_NEW.csv", star(* 0.10 ** 0.05 *** 0.01) replace style(html) cells(b(fmt(4) star) se(fmt(4) par)) title("Sample `sample', marginal effects: Shares of NEW coauthors, all genders") mlabels("New coauthors" "New senior" "New mid-career" "New junior" "New other" "New women" "New senior women" "New mid-career women" "New junior women" "New other women")
