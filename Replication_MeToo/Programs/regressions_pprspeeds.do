...
foreach sample in allaut onlyju senior {
	...
	foreach subsample in noslow nofast {
		use "${data}MeToo_author_ppr_`sample'.dta", clear
		
		merge m:1 unique_paper_id using "${data}pprs_speed_classification.dta", keep(match master) keepusing(nofast_ppr noslow_ppr) nogen
		
		do "${programs}define_relevant_shares.do"
		
		keep if  (`subsample'_ppr == 1)
			...
			if `j'==1 {
				...
				* glm without interactions with sex
				...
				{			
					...
					save "${intdata}`sample'_`subsample'_`var'.dta", replace
					restore
		...
		* Table with MEF of GLM of ALL COAUTHORS (2c-5c and 1d-5d)
		esttab mar_reg2c_mef_fr mar_reg3c_mef_fr mar_reg4c_mef_fr mar_reg5c_mef_fr mar_reg1d_mef_fr mar_reg2d_mef_fr mar_reg3d_mef_fr mar_reg4d_mef_fr mar_reg5d_mef_fr using "${output}tables_`sample'/`sample'_`subsample'_ALL_sexinteractions.csv", star(* 0.10 ** 0.05 *** 0.01) replace style(html) cells(b(fmt(4) star) se(fmt(4) par)) title("Sample `sample', marginal effects: Shares of ALL coauthors") mlabels("Senior" "Mid-career" "Junior" "Other" "Women" "Senior women" "Mid-career women" "Junior women" "Other women")
	
		* Table with MEF of GLM of ALL COAUTHORS, all genders (2c-5c and 1d-5d)
		esttab mar_reg2c_mef_all mar_reg3c_mef_all mar_reg4c_mef_all mar_reg5c_mef_all mar_reg1d_mef_all mar_reg2d_mef_all mar_reg3d_mef_all mar_reg4d_mef_all mar_reg5d_mef_all using "${output}tables_`sample'/`sample'_`subsample'_ALL.csv", star(* 0.10 ** 0.05 *** 0.01) replace style(html) cells(b(fmt(4) star) se(fmt(4) par)) title("Sample `sample', marginal effects: Shares of ALL coauthors, all genders") mlabels("Senior" "Mid-career" "Junior" "Other" "Women" "Senior women" "Mid-career women" "Junior women" "Other women")
	
		* Table with MEF of GLM of NEW COAUTHORS (1-5 and 1b-5b)
		esttab mar_reg1_mef_fr mar_reg2_mef_fr mar_reg3_mef_fr mar_reg4_mef_fr mar_reg5_mef_fr mar_reg1b_mef_fr mar_reg2b_mef_fr mar_reg3b_mef_fr mar_reg4b_mef_fr mar_reg5b_mef_fr using "${output}tables_`sample'/`sample'_`subsample'_NEW_sexinteractions.csv", star(* 0.10 ** 0.05 *** 0.01) replace style(html) cells(b(fmt(4) star) se(fmt(4) par)) title("Sample `sample', marginal effects: Shares of NEW coauthors") mlabels("New coauthors" "New senior" "New mid-career" "New junior" "New other" "New women" "New senior women" "New mid-career women" "New junior women" "New other women")
	
		* Table with MEF of GLM of NEW COAUTHORS, all genders (1-5 and 1b-5b)
		esttab mar_reg1_mef_all mar_reg2_mef_all mar_reg3_mef_all mar_reg4_mef_all mar_reg5_mef_all mar_reg1b_mef_all mar_reg2b_mef_all mar_reg3b_mef_all mar_reg4b_mef_all mar_reg5b_mef_all using "${output}tables_`sample'/`sample'_`subsample'_NEW.csv", star(* 0.10 ** 0.05 *** 0.01) replace style(html) cells(b(fmt(4) star) se(fmt(4) par)) title("Sample `sample', marginal effects: Shares of NEW coauthors, all genders") mlabels("New coauthors" "New senior" "New mid-career" "New junior" "New other" "New women" "New senior women" "New mid-career women" "New junior women" "New other women")
	}
}
